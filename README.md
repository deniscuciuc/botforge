![TeleForge](https://raw.githubusercontent.com/deniscuciuc/teleforge/main/assets/banner.png)

# TeleForge

[![CI](https://github.com/deniscuciuc/teleforge/actions/workflows/ci.yml/badge.svg)](https://github.com/deniscuciuc/teleforge/actions/workflows/ci.yml)
[![NuGet](https://img.shields.io/nuget/v/TeleForge.Core.svg?label=TeleForge.Core)](https://www.nuget.org/packages/TeleForge.Core/)
...
`Telegram.Bot` gives you an API client. TeleForge gives you the application around it: updates
arrive on one pipeline, replies leave on another, and both are yours to extend.

```
dotnet add package TeleForge.Hosting
```
...
| **Analyzers** | `TeleForge.Analyzers` catches what the compiler cannot: a handler with the wrong return type, a duplicate route, a handler class missing its attribute |
| **Testing** | `TeleForge.TestUtilities` gives you a fake client that records what was sent, and an update factory. No assertion framework attached |

## Packages

Start with `TeleForge.Hosting` — it pulls in routing, messaging and the consumer.

| Package | |
|---|---|
| `TeleForge.Hosting` | Host wiring: monolith, or split ingress and sender |
| `TeleForge.Core` | Update context, middleware contract, client provider |
| `TeleForge.Routing`, `.Routing.Abstractions` | Attribute routing and the update pipeline |
| `TeleForge.Messaging`, `.Messaging.Abstractions` | Message builder, send pipeline, in-memory queue |
| `TeleForge.Messaging.MassTransit` | RabbitMQ queue backend |
| `TeleForge.Consumer` | Long-polling ingress |
| `TeleForge.Templates` | YAML message templates |
| `TeleForge.Payments`, `.Payments.Abstractions` | The payments stack |
| `TeleForge.Payments.Crypto`, `.Payments.Fragment` | TON and Fragment payouts |
| `TeleForge.RateLimiting.Redis`, `.RateLimiting.Abstractions` | Distributed rate limiting |
| `TeleForge.Streaming` | Draft-message streaming |
| `TeleForge.Observability` | OpenTelemetry metrics |
| `TeleForge.Contracts` | Shared contracts for a split deployment |
| `TeleForge.Analyzers` | Roslyn analyzers and code fixes |
| `TeleForge.TestUtilities` | Test doubles |
| `TeleForge.Templates.DotNet` | `dotnet new teleforge-bot` and `dotnet new teleforge-handler` |
...
`TeleForge.Templates` resolves `{@key}` tokens through `ILocalizationKeyResolver`. Implement
it against whatever holds your translations — a database, resx, a spreadsheet — and register
it; the template engine picks it up. Nothing is bundled, so TeleForge does not pull a
