...
```bash
git clone https://github.com/deniscuciuc/teleforge.git
cd teleforge
dotnet restore
...
dotnet test tests/TeleForge.Integration.Tests -c Release
```
...
Two pipelines with a queue between them: updates come in through `TeleForge.Consumer` onto a
bounded channel, get routed by `TeleForge.Routing`, and replies go out through
`TeleForge.Messaging` via a queue backend and a send pipeline. That queue is what lets the
...
1. Add the attribute to `TeleForge.Routing.Abstractions/Attributes/` and the handler interface
...
4. Consider an analyzer rule in `tools/TeleForge.Analyzers` if the handler has a shape that is
   easy to get wrong.
5. Document it in `docs/routing.md`, and add an example bot if it needs one.

## Adding a package

Add the project to `TeleForge.slnx`, give it a `<Description>`, `<PackageTags>` and its own
...
Changes under `src/TeleForge.Payments*` need tests. This code decides whether money moves, and
its failure mode is quiet: a pre-checkout that approves when it should reject takes a real
payment for something that will not be delivered. `tests/TeleForge.Payments.Tests` covers the
payload serializer, the anti-fraud pipeline and the pre-checkout pipeline — extend it rather
than working around it.

## Documentation

Any C# you put in `README.md` must also exist in `tests/TeleForge.Docs.Snippets`, which
