...
public class ValidationTools(TemplateValidationService templateValidator)
{
    [McpServerTool]
    [Description(
        "Validates a TeleForge YAML template for structural correctness — required fields, translation consistency, button types, mustache balance, dynamic button configuration.")]
    ...
    [McpServerTool]
    [Description(
        "Validates a TeleForge handler implementation — checks that the correct interface is implemented, matching attribute is present, return type is correct, and DI patterns follow conventions.")]
