...
public sealed class EditMessageBurstScenario : IProbeScenario
{
    public string Name => "Edit Message Burst (Private)";

    public string Description =>
        "Send 1 message then edit it N times with no delay. Simulates crash-game rapid X-value updates in a private chat.";
    ...
    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        ...
        // Send the seed message that we'll keep editing
        ...
        if (seed.MessageId is null)
            throw new InvalidOperationException("Failed to send seed message — cannot run edit scenario.");
        ...
        await console.Progress()
            ...
            {
                var task = ctx.AddTask($"[bold]Edit burst → msg {messageId}[/]", maxValue: count);
