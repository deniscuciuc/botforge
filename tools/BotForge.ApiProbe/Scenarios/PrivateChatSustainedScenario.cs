...
public sealed class PrivateChatSustainedScenario : IProbeScenario
{
    public string Name => "Private Chat Sustained";

    public string Description =>
        "Ramp-up rate to a single private chat until 429s appear. Finds the sustainable threshold.";
    ...
    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        ...
        console.MarkupLine(
            $"[bold]Ramping from {defaults.RampStartRps} → +{defaults.RampStepRps}/step, step={defaults.RampStepDurationSeconds}s[/]");
