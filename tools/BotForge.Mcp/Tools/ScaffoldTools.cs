...
public static class ScaffoldTools
{
    [McpServerTool]
    [Description(
        "Generates a complete C# handler file for BotForge. Returns ready-to-use source code with correct interface, attribute, context, result, and using statements.")]
        ...
        string? chatTypes = null)
    {
        ...
        sb.AppendLine("using BotForge.Routing.Handlers;");
        sb.AppendLine("using BotForge.Routing.Attributes;");
        sb.AppendLine("using BotForge.Messaging.Abstractions;");
        sb.AppendLine();
        ...
        switch (handlerType.ToLowerInvariant())
        {
            ...
            case "conversation":
                ...
                sb.Clear();
                sb.AppendLine("using BotForge.Routing.Abstractions;");
                sb.AppendLine("using BotForge.Messaging.Abstractions;");
                sb.AppendLine();
                sb.AppendLine($"namespace {ns};");
                sb.AppendLine();
    ...
    [McpServerTool]
    [Description(
        "Generates a YAML template file for BotForge with translations, optional static buttons, and optional dynamic buttons with pagination.")]
    ...
    [McpServerTool]
    [Description(
        "Generates a middleware class for the BotForge update pipeline (ITelegramMiddleware) or send pipeline (ISendMiddleware).")]
        ...
        string ns = "MyBot.Payments")
    {
        ...
        sb.AppendLine("using BotForge.Payments.Abstractions;");
        sb.AppendLine();
        sb.AppendLine($"namespace {ns};");
        sb.AppendLine();
    ...
    [McpServerTool]
    [Description(
        "Generates a complete bot project structure — Program.cs, appsettings.json, and csproj content — for a new BotForge bot.")]
        ...
        string consumerMode = "polling")
    {
        ...
        sb.AppendLine("    <PackageReference Include=\"BotForge.Consumer\" />");
        sb.AppendLine("    <PackageReference Include=\"BotForge.Routing\" />");
        sb.AppendLine("    <PackageReference Include=\"BotForge.Messaging\" />");
        if (useTemplates)
            sb.AppendLine("    <PackageReference Include=\"BotForge.Templates\" />");
        if (usePayments)
            sb.AppendLine("    <PackageReference Include=\"BotForge.Payments\" />");
        ...
        sb.AppendLine("using BotForge.Consumer.Extensions;");
        sb.AppendLine("using BotForge.Routing.Extensions;");
        sb.AppendLine("using BotForge.Messaging;");
        if (useTemplates)
            sb.AppendLine("using BotForge.Templates;");
        if (usePayments)
            sb.AppendLine("using BotForge.Payments;");
        sb.AppendLine("using BotForge.Core;");
        sb.AppendLine();
