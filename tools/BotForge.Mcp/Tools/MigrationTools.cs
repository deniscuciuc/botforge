...
public class MigrationTools(RoslynAnalysisService roslyn)
{
    [McpServerTool]
    [Description(
        "Audits a single C# source file for Telegram.Bot API patterns and suggests BotForge equivalents. Provide either a file path or the source code directly.")]
        ...
        string directoryPath)
    {
        ...
        sb.AppendLine("1. Create new BotForge project with `scaffold_bot_project` tool");
    ...
    [McpServerTool]
    [Description(
        "Analyzes a specific method or class from a custom bot and suggests the best BotForge handler type, including a transformed code example.")]
    public string SuggestHandlerMapping(
        [Description("C# source code of the method or class to map to a BotForge handler")]
        string sourceCode)
    {
        if (string.IsNullOrWhiteSpace(sourceCode))
