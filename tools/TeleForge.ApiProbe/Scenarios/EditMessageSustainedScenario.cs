...
public sealed class EditMessageSustainedScenario : IProbeScenario
{
    public string Name => "Edit Message Sustained (Private)";

    public string Description =>
        "Ramp-up edit rate on a single message in a private chat. Finds sustainable edits/second threshold.";
    ...
    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        ...
        console.MarkupLine("[bold]Sending seed message...[/]");
        var seed = await context.SendProbeMessageAsync(bot, botName, chatId, 0, ct);
        if (seed.MessageId is null)
            throw new InvalidOperationException("Failed to send seed message.");
        ...
        console.MarkupLine(
            $"[bold]Ramping edits from {defaults.RampStartRps} → +{defaults.RampStepRps}/step, step={defaults.RampStepDurationSeconds}s[/]");
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            console.MarkupLine($"  [cyan]Step: {currentRps} edit/s[/]");
            ...
            while (DateTimeOffset.UtcNow - stepStart < stepDuration)
            {
                ...
                var record = await context.EditProbeMessageAsync(bot, botName, chatId, messageId, index++, ct);
                records.Add(record);
                stepRecords.Add(record);

                if (record.IsRateLimited)
                    console.MarkupLine(
                        $"    [yellow]429 at edit #{record.Index} — retry_after={record.RetryAfterSeconds}s[/]");
            }
            ...
            if (rateLimitedPercent > 10)
            {
                ...
                console.MarkupLine(
                    $"  [red]Threshold reached at ~{thresholdRps} edit/s (>10% rate limited at {currentRps} edit/s)[/]");
                break;
            }
            ...
            if (currentRps > 100)
            {
                console.MarkupLine("[yellow]Safety cap at 100 edit/s. Stopping ramp.[/]");
                break;
            }
        }
