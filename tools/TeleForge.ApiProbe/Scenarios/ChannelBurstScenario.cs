...
public sealed class ChannelBurstScenario : IProbeScenario
{
    public string Name => "Channel Burst";

    public string Description =>
        "Fire N messages to a channel with no delay. Finds the hard per-channel message limit.";
    ...
    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        ...
        await console.Progress()
            ...
            {
                var task = ctx.AddTask($"[bold]Channel burst → chat {chatId}[/]", maxValue: count);
