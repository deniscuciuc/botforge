...
public sealed class ChannelSustainedScenario : IProbeScenario
{
    public string Name => "Channel Sustained";
    public string Description => "Ramp-up message rate to a channel. Finds the sustainable messages/second threshold.";
    ...
    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        ...
        console.MarkupLine(
            $"[bold]Channel ramp: {defaults.RampStartRps} → +{defaults.RampStepRps}/step, step={defaults.RampStepDurationSeconds}s[/]");
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (currentRps > 100)
            {
                console.MarkupLine("[yellow]Safety cap at 100 msg/s for channel. Stopping ramp.[/]");
                break;
            }
        }
