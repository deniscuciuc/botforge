...
public sealed class DiceBurstScenario : IProbeScenario
{
    public string Name => "Dice Burst (Private)";

    public string Description =>
        "Burst-send 🎰 slot machine dice to a private chat. Finds the hard per-chat dice limit.";
    ...
    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        ...
        await console.Progress()
            ...
            {
                var task = ctx.AddTask($"[bold]🎰 Dice burst → chat {chatId}[/]", maxValue: count);
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (record.IsRateLimited)
                        console.MarkupLine($"  [yellow]429 at dice #{i} — retry_after={record.RetryAfterSeconds}s[/]");
                }
            });
