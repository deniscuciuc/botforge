...
public sealed class EditMessageChannelBurstScenario : IProbeScenario
{
    public string Name => "Edit Message Burst (Channel)";
    public string Description => "Send 1 message to channel then edit it N times. Tests edit rate limits in channels.";

    public async Task<ScenarioResult> RunAsync(ProbeContext context, IAnsiConsole console, CancellationToken ct)
    {
        var chatId = context.Config.ChannelChatId;
        if (chatId == 0)
            throw new InvalidOperationException("ChannelChatId must be set in config.");
        ...
        console.MarkupLine("[bold]Sending seed message to channel...[/]");
        var seed = await context.SendProbeMessageAsync(bot, botName, chatId, 0, ct);
        if (seed.MessageId is null)
            throw new InvalidOperationException("Failed to send seed message to channel.");
        ...
        await console.Progress()
            ...
            {
                var task = ctx.AddTask($"[bold]Edit burst (channel) → msg {messageId}[/]", maxValue: count);
