...
using TeleForge.Messaging.MassTransit;
...
public static class MonolithHostExtensions
{
    /// <summary>
    /// Registers all framework services for a monolith deployment (Ingress + Sender in one process).
    /// Uses in-memory queue by default; switch to RabbitMQ via configuration.
    /// </summary>
    public static IServiceCollection AddTelegramMonolithHost(
        this IServiceCollection services,
        IConfiguration configuration,
        Action<MonolithHostOptions>? configure = null)
    {
        ArgumentNullException.ThrowIfNull(configuration);

        var options = new MonolithHostOptions();
        configuration.GetSection("Telegram").Bind(options);
        configure?.Invoke(options);

        ConfigureMessaging(services, options.Bots, options.Queue, options.Send);
    ...
    private static void ConfigureRouting(IServiceCollection services, List<string> handlerAssemblies)
    {
        ...
        {
            routing.AddHandlersFromAssembly(typeof(MonolithHostExtensions).Assembly);
