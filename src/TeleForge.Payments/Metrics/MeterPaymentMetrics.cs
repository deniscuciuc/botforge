...
namespace TeleForge.Payments.Metrics;
...
public sealed class MeterPaymentMetrics : IPaymentMetrics
{
    ...
    public const string MeterName = "TeleForge.Payments";

    private static readonly Meter Meter = new(MeterName);

    private static readonly Counter<long> InvoiceCreatedCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.invoice.created", "{invoice}",
            "Created invoices.");

    private static readonly Counter<long> CheckoutCompletedCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.checkout.completed", "{checkout}",
            "Pre-checkout approvals/rejections.");

    private static readonly Counter<long> PaymentOutcomeCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.payment.outcome", "{payment}",
            "Payment success/failure outcomes.");

    private static readonly Histogram<long> PaymentAmountHistogram =
        Meter.CreateHistogram<long>("teleforge.telegram.payments.payment.amount", "star",
            "Payment amount in stars.");

    private static readonly Counter<long> RefundProcessedCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.refund.processed", "{refund}",
            "Processed refunds.");

    private static readonly Histogram<long> RefundAmountHistogram =
        Meter.CreateHistogram<long>("teleforge.telegram.payments.refund.amount", "star",
            "Refund amount in stars.");

    private static readonly Counter<long> PayoutRequestedCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.payout.requested", "{payout}",
            "Requested payouts.");

    private static readonly Counter<long> PayoutCompletedCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.payout.completed", "{payout}",
            "Completed payouts by status.");

    private static readonly Histogram<double> PayoutDurationMsHistogram =
        Meter.CreateHistogram<double>("teleforge.telegram.payments.payout.duration", "ms",
            "Payout duration in milliseconds.");

    private static readonly Counter<long> GiftSentCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.gift.sent", "{gift}",
            "Sent gifts.");

    private static readonly Counter<long> PaidMediaSoldCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.paid_media.sold", "{sale}",
            "Paid media sales.");

    private static readonly Histogram<long> PaidMediaStarsHistogram =
        Meter.CreateHistogram<long>("teleforge.telegram.payments.paid_media.stars", "star",
            "Paid media stars sold per transaction.");

    private static readonly Counter<long> AntiFraudCheckCounter =
        Meter.CreateCounter<long>("teleforge.telegram.payments.antifraud.check", "{check}",
            "Anti-fraud checks and outcomes.");

    private static readonly Histogram<double> PaymentProcessingDurationMs =
        Meter.CreateHistogram<double>("teleforge.telegram.payments.processing.duration", "ms",
