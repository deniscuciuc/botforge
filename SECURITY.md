...
Only the latest patch of the latest minor release receives security fixes. All TeleForge
packages are versioned and released together, so a fix ships as a new patch across all of
them.

## Scope

In scope: the TeleForge packages in this repository. The highest-value targets, roughly in
order:

- **`TeleForge.Payments`** — anything that could approve a pre-checkout that should have been
...
- **`TeleForge.Routing`** — authorization attributes and the middleware pipeline. A route that
  reaches a handler with `[Authorize]` unsatisfied is a vulnerability.
- **`TeleForge.RateLimiting.Redis`** — a limit that can be evaded across instances.
- **`TeleForge.Templates`** — untrusted values reaching a template in a way that changes its
