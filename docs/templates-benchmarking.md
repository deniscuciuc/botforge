...
Production-grade benchmarking for the `BotForge.Templates` template engine.
...
dotnet run --project tools/BotForge.Templates.Benchmarks -c Release -- --filter *
```
...
```bash
dotnet run --project tools/BotForge.Templates.Benchmarks -c Release -- \
  --filter * --join
```
...
```
tools/BotForge.Templates.Benchmarks/
