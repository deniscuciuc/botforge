...
# Or manually:
docker build -t telegram-ingress:local -f services/TeleForge.Ingress/Dockerfile .
docker build -t telegram-sender:local -f services/TeleForge.Sender/Dockerfile .
```
...
`TeleForge.Hosting`, and most deployments want their own handlers compiled in anyway. Build
them from the Dockerfiles in `services/`:

```bash
docker build -f services/TeleForge.Ingress/Dockerfile -t your-registry/teleforge-ingress:1.0.0 .
docker build -f services/TeleForge.Sender/Dockerfile  -t your-registry/teleforge-sender:1.0.0 .
```
