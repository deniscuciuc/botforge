...
TeleForge is a modular framework built on the **Dependency Inversion Principle** — all contracts live in `*Abstractions` packages, while implementations are in separate packages. This allows consuming code to reference only the abstractions and swap implementations (e.g., in-memory → Redis rate limiting, channel → MassTransit queuing).
...
```
TeleForge.Core (foundation)
│
├── TeleForge.Routing.Abstractions
│   └── TeleForge.Routing (handler discovery, middleware, authorization)
│
├── TeleForge.Messaging.Abstractions
│   ├── TeleForge.Messaging (send pipeline, builder, transport)
│   └── TeleForge.Messaging.MassTransit (distributed queue)
│
├── TeleForge.RateLimiting.Abstractions
│   └── TeleForge.RateLimiting.Redis (Redis sliding window)
│
├── TeleForge.Payments.Abstractions
│   ├── TeleForge.Payments (checkout, invoices, refunds, payouts)
│   └── TeleForge.Payments.Fragment (TON payout provider)
│
├── TeleForge.Templates (YAML engine, keyboard builder, localization)
│
└── TeleForge.Consumer (polling, webhook, hosted service)
```
