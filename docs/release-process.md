...
2. Add a policy for the `BotForge` package prefix, owner `deniscuciuc`, repository
   `deniscuciuc/botforge`, workflow `release.yml`, environment `nuget`.
...
else. There are 19 rather than 21 symbol packages because `BotForge.Analyzers` and
`BotForge.Templates.DotNet` ship no `lib/` output.
